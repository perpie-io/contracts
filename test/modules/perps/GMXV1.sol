...
contract GMXV1FeesModuleTest is ArbiTest {
    ...
    IERC20Metadata wbtc = IERC20Metadata(0xf97f4df75117a78c1A5a0DBb814Af92458539FB4);
    ...
    function setUp() public virtual override {
        ...
        vm.prank(entryPoint);
        ModularAccount(address(smartAccount)).enableModule(address(gmxv1FeeModule));
    ...
    function testIncreasePosition(uint256 _sizeDelta, bool _isLong, uint8 leverage) external {
        ...
        uint256 positionRouterBalanceBefore = usdt.balanceOf(address(positionRouter));
        ...
        uint256 chargedFee = usdt.balanceOf(address(feesManager)) - feesManagerBalanceBefore;
        ...
        assertEq(usdt.balanceOf(smartAccount), 0, "Increase Position - Smart Account has USDT left after increase");
    }

    function testIncreasePositionEth(uint256 _sizeDelta, bool _isLong, uint8 leverage) external {
        _assumeRealisticInputs(leverage, _sizeDelta, weth);

        uint256 amountIn = _getAmountIn(_sizeDelta, leverage, _isLong, weth);

        uint256 positionRouterBalanceBefore = weth.balanceOf(address(positionRouter));
        ...
        uint256 chargedFee = address(feesManager).balance - feesManagerBalanceBefore;

        assertGt(chargedFee, 0, "Charged fee is 0");

        assertEq(expectedFee, chargedFee, _makeError("Increase Position ETH - Charged fee is not the expected fee"));
        ...
        assertEq(
            ...
            positionRouterBalanceBefore + expectedInputAmount,
            _makeError("Increase Position ETH - PositionRouter did not receive correct amountIn")
        );

        assertEq(smartAccount.balance, 0, "Increase Position ETH - Smart Account has ETH left after increase");
    }

    function testIncreaseOrder(uint256 _sizeDelta, bool _isLong, uint8 leverage) external {
        _assumeRealisticInputs(leverage, _sizeDelta, usdt);
        ...
        assertEq(expectedFee, chargedFee, _makeError("Increase Order - Charged fee is not the expected fee"));
        ...
        uint256 orderbookBalanceDiff = usdt.balanceOf(address(orderbook)) - orderbookBalanceBefore;
        uint256 vaultBalanceDiff = usdt.balanceOf(address(vault)) - vaultBalanceBefore;

        assertEq(
            _isLong ? vaultBalanceDiff : orderbookBalanceDiff,
            expectedInputAmount,
            _makeError("Increase Order - vault did not receive correct amountIn")
        );

        assertEq(smartAccount.balance, 0, "Increase Order - Smart Account has USDT left after increase");
    }

    function _increasePosition(uint256 _sizeDelta, bool _isLong, uint256 amountIn, bool expectRevert) internal {
        address[] memory path = _makePath(address(usdt), address(wbtc), _isLong);
        ...
        gmxv1FeeModule.createIncreasePosition{value: executionFee}(
            path, address(wbtc), amountIn, 0, _sizeDelta, _isLong, 0, executionFee, bytes32(0), address(0)
        );
        vm.stopPrank();
    }

    function _increasePositionETH(uint256 _sizeDelta, bool _isLong, uint256 amountIn, bool expectRevert) internal {
        address[] memory path = _makePath(address(weth), address(wbtc), _isLong);
        ...
        gmxv1FeeModule.createIncreasePositionETH{value: executionFee + amountIn}(
            path, address(wbtc), 0, _sizeDelta, _isLong, 0, executionFee, bytes32(0), address(0)
        );
        vm.stopPrank();
    }

    function _increaseOrder(uint256 _sizeDelta, bool _isLong, uint256 amountIn, bool expectRevert) internal {
        uint256 executionFee = _executionFee();
    ...
    function _makePath(address inputToken, address indexToken, bool isLong)
        internal
        pure
    ...
    function _executionFee() internal view returns (uint256 executionFee) {
        ...
        return max(positionRouter.minExecutionFee(), orderbook.minExecutionFee());
    }
    ...
    function estimateFee(uint256 sizeDelta, bool isLong, IERC20Metadata token) internal view returns (uint256) {
        ...
        return (sizeFee * 10 ** token.decimals()) / (_getPrice(address(token), isLong));
    }
    ...
    function _getPrice(address token, bool isLong) internal view returns (uint256) {
        return isLong ? vault.getMinPrice(address(token)) : vault.getMaxPrice(address(token));
    }
